...
const learningRateLimit = 1 * time.Hour

// ContributeLearning creates a GitHub PR with knowledge the mayor wants to
// share back to the template. Rate limited to 1 PR per mayor per hour.
func (m *Manager) ContributeLearning(worldID, title, content, filePath string) error {
	// TODO: Track last PR time per world in mayor_activity, enforce rate limit.
	...
	commands := [][]string{
		...
		{"git", "add", filePath},
		{"git", "commit", "-m", fmt.Sprintf("Mayor learning: %s\n\nContributed by mayor of world %s", title, worldID)},
		{"git", "push", "-u", "origin", branchName},
		{"gh", "pr", "create", "--title", title, "--body", fmt.Sprintf("Contributed by the mayor of world `%s`.\n\n%s", worldID, content)},
		{"git", "checkout", "main"},
	}

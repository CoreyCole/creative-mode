...
if wait $pid_go; then
  ...
  echo "FAIL  harness (golangci-lint)"
  cat "$go_out"
  failed=1
fi
...
if wait $pid_clippy_server; then
  ...
  echo "FAIL  template/server+shared (clippy)"
  cat "$rs_clippy_server_out"
  failed=1
fi
...
if wait $pid_clippy_client; then
  ...
  echo "FAIL  template/client (clippy wasm)"
  cat "$rs_clippy_client_out"
  failed=1
fi

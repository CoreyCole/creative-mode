...
func (m *Manager) provisionAgent(
	...
	onboarding interface{}, // *worldchannel.OnboardingData or nil
) error {
	workspaceDir := filepath.Join(m.openclawHome, "workspaces", agentID)

	// Write workspace files.
	if err := writeWorkspaceFiles(workspaceDir, worldID, worldName, mayorName, mayorSecret, m.harnessURL, onboarding); err != nil {
...
func (m *Manager) bindAgentToDiscord(agentID, channelID string) error {
	...
	bindings = append(bindings, map[string]any{
		"agent":   agentID,
		"channel": fmt.Sprintf("discord:%s", channelID),
	})
...
func (m *Manager) deleteAgent(agentID string) error {
	...
	cmd := exec.CommandContext(ctx, m.openclawBin, "agents", "delete", "--id", agentID)
	cmd.Env = append(cmd.Environ(), "OPENCLAW_HOME="+m.openclawHome)
